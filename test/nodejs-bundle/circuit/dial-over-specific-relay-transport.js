  ...
  function setUpNodes (muxer, done) {
    utils.createNodes(
      ...
      (err, nodes) => {
        ...
        testNodes = nodes
        relaySpy1 = sinon.spy(testNodes['relayNode1'].swarm.transports[Circuit.name].listeners[0].hopHandler, 'handle')
        relaySpy2 = sinon.spy(testNodes['relayNode2'].swarm.transports[Circuit.name].listeners[0].hopHandler, 'handle')
  ...
  it('dial over the correct chained relay addr', function (done) {
    utils.dialAndReverse(testNodes['nodeA'], testNodes['nodeB'], ['hello'], (err, result) => {
      ...
      expect(relaySpy1.args.some((a) => {
        return a[0] &&
          a[0].dstPeer &&
          a[0].dstPeer.addrs[0].toString() === `/ipfs/${testNodes['nodeB'].peerInfo.id.toB58String()}`
      })).to.be.ok()
